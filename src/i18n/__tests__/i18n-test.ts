...
describe('avatar studio translations', () => {
  ...
  it('translates the talking preview controls', () => {
    expect(translateStudioText('Faire parler', 'en')).toBe('Talk')
    expect(translateStudioText('Arrêter de parler', 'zh-CN')).toBe('停止说话')
    expect(translateStudioText('Réfléchir', 'en')).toBe('Think')
    expect(translateStudioText('Arrêter de réfléchir', 'zh-CN')).toBe('停止思考')
  })
