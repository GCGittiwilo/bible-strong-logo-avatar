...
const defaultPlayback: StatePlaybackSelection = { stateId: 'ready', playing: true }
...
export const parseStudioDocument = (value: unknown, fallback: StudioDocument): StudioDocument => {
  ...
  const storedBundledRevision = candidate.library?.bundledAvatarRevision ?? 0
  const fallbackBundledRevision = fallback.library.bundledAvatarRevision ?? 0
  const useBundledChatPipeline = fallbackBundledRevision > storedBundledRevision
  const expressions =
    !useBundledChatPipeline && Array.isArray(candidate.expressions) && candidate.expressions.length
      ? parseExpressions(candidate.expressions)
      : fallback.expressions
  const sequences =
    !useBundledChatPipeline && Array.isArray(candidate.sequences)
  ...
  return {
    ...
    expressions,
    sequences,
    playback: useBundledChatPipeline
      ? { ...fallback.playback }
      : parsePlayback(candidate.playback, fallback.playback),
  }
}
