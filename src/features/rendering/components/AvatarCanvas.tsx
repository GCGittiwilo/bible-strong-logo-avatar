...
import type { AvatarMouth } from '@/features/avatar/mouth'
...
export function AvatarCanvas({
  ...
  faceForward?: boolean
  faceReveal: MotionValue<number>
  statusEffect?: SequenceEffect
  ...
  const faceScale = useTransform(faceReveal, [0, 1], [0.84, 1])
  ...
  return (
    <div className="avatar-wrap">
      ...
      >
          ...
          <motion.g style={{ x: offsetX, y: offsetY }}>
            {backPaths.map((pathValue, index) => (
              <motion.path
                ...
                d={pathValue}
                key={index}
                opacity={backPathOpacities[index]}
            ...
            <motion.path
              className={`avatar-head ${highlight === 'head' ? 'cyan-outline' : ''}`}
              d={headPath}
              fillOpacity={logoMorph?.primaryOpacity ?? 1}
              onPointerDown={event => {
                onBodyNodeSelect('primary')
                startDrag(event)
              }}
            ...
            >
              ...
              />
              {mouth && (
                <motion.g className="avatar-mouth" opacity={mouthOpacity} pointerEvents="none">
                  <motion.path d={mouthPath} fill={mouth.color} />
                  <motion.path d={tonguePath} fill={mouth.tongueColor} />
                </motion.g>
              )}
            </motion.g>
            {frontPaths.map((pathValue, index) => (
              <motion.path
                ...
                d={pathValue}
                key={index}
                opacity={frontPathOpacities[index]}
        ...
        {statusEffect && (
          <motion.g
            className="avatar-status-effect"
            aria-hidden="true"
            pointerEvents="none"
            style={{ x: statusAnchorX, y: statusAnchorY }}
          >
            <StatusEffectGlyph effect={statusEffect} />
          </motion.g>
        )}
        {selectedBodyPath && (
