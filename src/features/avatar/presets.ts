...
const reactiveStates = new Set(['speaking', 'complete', 'error'])
...
export const getStatePlaybackConfig = (name: string): StatePlaybackConfig => {
  ...
  return {
    expressionIntervalMs: name === 'ready' ? 5200 : 2300,
    blink: { ...blink },
  }
}

export const stateNotes: Record<string, string> = {
  ready: 'Présence neutre et disponible avant le prochain message.',
  listening: 'Expressions 10, 01 et 19, regard stable et clignement attentif.',
  transcribing: 'Regard concentré pendant la conversion de la voix en texte.',
  ...
  speaking: 'Expression vivante et rythmée pendant la réponse vocale.',
  complete: 'Confirmation positive lorsque la réponse est terminée.',
  error: 'Alerte lisible lorsqu’une étape de la conversation échoue.',
}
