...
import { idleComicMouthPose, type AvatarMouth, type ComicMouthPose } from './mouth'
...
/** Keeps the animated eye relationship intact while anchoring the pair around the face center. */
export const centerFaceExpression = (expression: Expression): Expression => {
  const centerX = (expression.positionXLeft + expression.positionXRight) / 2
  const centerY = (expression.positionYLeft + expression.positionYRight) / 2
  return {
    ...expression,
    positionXLeft: expression.positionXLeft - centerX,
    positionXRight: expression.positionXRight - centerX,
    positionYLeft: expression.positionYLeft - centerY,
    positionYRight: expression.positionYRight - centerY,
  }
}
...
const comicMouthGeometry = (
  pose: AvatarPose,
  surface: SurfaceConfig,
  mouth: AvatarMouth | undefined,
  mouthPose: ComicMouthPose = idleComicMouthPose
) => {
  if (!mouth) return { mouthPath: '', tonguePath: '', mouthVisible: false }
  if (mouth.style === 'polished') {
    const openness = Math.max(0, Math.min(1, mouthPose.openness))
    const halfWidth = (mouth.width * Math.max(0.62, mouthPose.widthScale)) / 2
    const openHeight = mouth.height * 0.52 * openness
    const lipThickness = 2.35 + openness * 0.65
    const tilt = radians(mouthPose.tilt * 0.22)
    const centerX = mouth.positionX + mouthPose.offsetX * 0.32
    const centerY = mouth.positionY + mouthPose.offsetY * 0.38
    const sample = (localX: number, localY: number) => {
      const rotatedX = localX * Math.cos(tilt) - localY * Math.sin(tilt)
      const rotatedY = localX * Math.sin(tilt) + localY * Math.cos(tilt)
      return projectFacePoint(pose, surface, centerX + rotatedX, centerY + rotatedY)
    }
    const edgeSamples = 25
    const upper = Array.from({ length: edgeSamples }, (_, index) => {
      const normalizedX = -1 + (index / (edgeSamples - 1)) * 2
      const centerWeight = 1 - normalizedX * normalizedX
      const smile = centerWeight * (5.2 - openness * 4.2)
      return sample(
        normalizedX * halfWidth,
        smile - lipThickness / 2 - openHeight * centerWeight * 0.43
      )
    })
    const lower = Array.from({ length: edgeSamples }, (_, index) => {
      const normalizedX = 1 - (index / (edgeSamples - 1)) * 2
      const centerWeight = 1 - normalizedX * normalizedX
      const smile = centerWeight * (5.2 - openness * 4.2)
      return sample(
        normalizedX * halfWidth,
        smile + lipThickness / 2 + openHeight * centerWeight * 0.57
      )
    })
    const mouthSamples = [...upper, ...lower]
    const tongueWidth = halfWidth * (0.46 + mouthPose.tongue * 0.08)
    const tongueHeight = Math.max(1.1, openHeight * 0.2 * mouthPose.tongue)
    const tongueCenterY = openHeight * 0.34 + 2
    const tongueSamples =
      mouthPose.tongue <= 0.04 || openness < 0.32
        ? []
        : Array.from({ length: 28 }, (_, index) => {
            const angle = (index / 28) * Math.PI * 2
            return sample(
              Math.cos(angle) * tongueWidth,
              tongueCenterY + Math.sin(angle) * tongueHeight
            )
          })
    return {
      mouthPath: path(mouthSamples.map(item => item.point)),
      tonguePath: path(tongueSamples.map(item => item.point)),
      mouthVisible:
        mouthSamples.reduce((total, samplePoint) => total + samplePoint.normal[2], 0) > 0,
    }
  }
  const halfWidth = (mouth.width * Math.max(0.45, mouthPose.widthScale)) / 2
  const halfHeight = Math.max(2.4, 2.4 + mouth.height * 0.48 * mouthPose.openness)
  const tilt = radians(mouthPose.tilt)
  const centerX = mouth.positionX + mouthPose.offsetX
  const centerY = mouth.positionY + mouthPose.offsetY
  const sample = (localX: number, localY: number) => {
    const rotatedX = localX * Math.cos(tilt) - localY * Math.sin(tilt)
    const rotatedY = localX * Math.sin(tilt) + localY * Math.cos(tilt)
    return projectFacePoint(pose, surface, centerX + rotatedX, centerY + rotatedY)
  }
  const mouthSamples = Array.from({ length: 32 }, (_, index) => {
    const angle = (index / 32) * Math.PI * 2
    const organic = 1 + Math.sin(angle * 3 + mouthPose.wobble * Math.PI) * 0.045
    const asymmetricY = Math.sin(angle) * halfHeight * (1 + Math.cos(angle) * mouthPose.wobble)
    return sample(Math.cos(angle) * halfWidth * organic, asymmetricY)
  })
  const tongueWidth = halfWidth * (0.5 + mouthPose.tongue * 0.08)
  const tongueHeight = Math.max(1.2, halfHeight * 0.3 * mouthPose.tongue)
  const tongueCenterY = halfHeight * 0.48
  const tongueSamples =
    mouthPose.tongue <= 0.02
      ? []
  ...
  return {
    mouthPath: path(mouthSamples.map(item => item.point)),
    tonguePath: path(tongueSamples.map(item => item.point)),
    mouthVisible: mouthSamples.reduce((total, samplePoint) => total + samplePoint.normal[2], 0) > 0,
  }
}
