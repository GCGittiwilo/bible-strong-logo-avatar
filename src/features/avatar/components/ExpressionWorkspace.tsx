...
export function ExpressionPreview({
  ...
  const faceVisible = logoMorph ? !showLogo : true
  const nodeOpacity = (nodeId: string | null | undefined) =>
    faceVisible && centerNodeIds.has(nodeId ?? '') ? 0 : 1
  return (
    <svg viewBox="-150 -150 300 300" aria-hidden="true">
      <defs>
        <clipPath id={clipId}>
      ...
      {geometry.backPaths.map((pathValue, index) => (
        <path
          ...
          d={pathValue}
          key={index}
          opacity={nodeOpacity(geometry.backNodeIds[index])}
      ...
      {geometry.frontPaths.map((pathValue, index) => (
        <path
          ...
          d={pathValue}
          key={`front-${index}`}
          opacity={nodeOpacity(geometry.frontNodeIds[index])}
          style={{ fill: resolvedColors.body }}
        />
      ))}
      {effect && faceVisible && (
        <g className="preview-status-effect" transform="translate(88 -88) scale(.66)">
          <StatusEffectGlyph effect={effect} />
        </g>
      )}
    </svg>
  )
