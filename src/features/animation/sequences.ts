...
const parseSequence = (value: unknown, fallback: AvatarSequence): AvatarSequence => {
  ...
  return {
    ...
    ...(presentations.includes(candidate?.presentation as SequencePresentation)
      ? { presentation: candidate?.presentation as SequencePresentation }
...
export const parseSequences = (value: unknown): AvatarSequence[] => {
  ...
  const fallback = defaults.find(sequence => sequence.id === 'ready') ?? defaults[0]
